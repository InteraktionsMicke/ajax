    ...
    for (i = 0; i < jobList.length; i++) {
        jobInfo +=
            ...
            <div class = "jobWrapper" id="wrapper${[i]}" value="${jobList[i].annonsid}" >
                ...
                <p class = "ansokningsdag"> Sista ansökninsdag: ${jobList[i].sista_ansokningsdag.substring(0,10)} </p>
                <p class = "publiceringsdatum"> Publicerad: ${jobList[i].publiceraddatum.substring(0,10)} </p>
...
//looping out pagination based on number of avalible ads
function createPagination(allJobData) {

    var pageNumber = 1;
    var allJobList = allJobData.matchningslista.matchningdata


    for (i = 0; i < allJobList.length; i += 20) {

        var paginationItem = document.createElement("li");
        document.getElementById("paginationList").appendChild(paginationItem);

        var page = document.createElement("SPAN");

        page.addEventListener('click', getJobs.bind(this, pageNumber, undefined));

        var pageValue = document.createTextNode(`${pageNumber}`);
        page.appendChild(pageValue);

        paginationItem.appendChild(page)
